...
"""Canvas template running Claude Code from flattened ChatKit inputs."""
...
class FlattenChatPromptNode(TaskNode):
    """Build one Claude prompt from ChatKit history plus the latest message."""

    async def run(self, state: State, config: RunnableConfig) -> dict[str, Any]:
        """Return a prompt string composed from the current inputs."""
        del config
...
class PublishExternalAgentReplyNode(TaskNode):
    ...
    async def run(self, state: State, config: RunnableConfig) -> dict[str, Any]:
        """Return the most recent non-empty agent output as text."""
        del config
    ...
    async def __call__(self, state: State, config: RunnableConfig) -> dict[str, Any]:
        """Wrap run() and expose its result as an AIMessage when available."""
...
def build_graph() -> StateGraph:
    """Build the Claude Code template workflow."""
    ...
    graph.add_node(
        ...
        PublishExternalAgentReplyNode(
            name="publish_reply",
            source_result_key="claude_code_agent",
        ),
    )

    graph.add_edge(START, "prepare_prompt")
    graph.add_edge("prepare_prompt", "claude_code_agent")
    graph.add_edge("claude_code_agent", "publish_reply")
