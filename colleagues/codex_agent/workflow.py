...
"""Canvas template running Codex from flattened ChatKit inputs."""
...
class FlattenChatPromptNode(TaskNode):
    """Build one Codex prompt from ChatKit history plus the latest message."""
...
def build_graph() -> StateGraph:
    """Build the Codex template workflow."""
    ...
    graph.add_node(
        ...
        PublishExternalAgentReplyNode(
            name="publish_reply",
            source_result_key="codex_agent",
        ),
    )

    graph.add_edge(START, "prepare_prompt")
    graph.add_edge("prepare_prompt", "codex_agent")
    graph.add_edge("codex_agent", "publish_reply")
